...
const Services = () => {
  ...
  const categories = mockServices.reduce((acc, service) => {
  ...
  const handleProceedToBooking = () => {
    ...
    if (additionalServices.length > 0) {
      ...
      finalPrice += additionalServices.reduce((acc, s) => acc + s.price, 0);
    }
  ...
  const suggestedServices = selectedService 
    ? mockServices.filter(s => s.id !== selectedService.id).slice(0, 4)
    : [];
              ...
              <div className="bg-gray-50 border-t border-gray-100 p-6 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-extrabold text-gray-900">
                    ₹{selectedService.price + additionalServices.reduce((acc, s) => acc + s.price, 0)}
                  </div>
                </div>
