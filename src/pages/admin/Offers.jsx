...
import { FaEdit, FaTrash, FaPlus, FaSave, FaTimes, FaSpinner, FaChevronDown, FaCheckSquare, FaRegSquare, FaClock, FaCalendarAlt } from 'react-icons/fa';
...
const AdminOffers = () => {
  ...
  const handleOpenEdit = (offer) => {
    ...
    setCurrentOffer({ 
      ...offer, 
      services: offer.services || [] // Fallback if no services exist
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      if (modalMode === 'add') {
        ...
        setOffers(offers.map(o => o.id === currentOffer.id ? currentOffer : o));
      }
      setIsLoading(false);
      handleCloseModal();
    }, 1200);
        ...
        <AnimatePresence>
          {offers.map((offer) => (
            ...
            >
              ...
              <div className="p-8 pt-12 relative z-10 flex-1 flex flex-col">
                ...
                {/* Custom Services rendering if array exists, else use description */}
                {offer.services && offer.services.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-600 mb-6 line-clamp-2 flex-1">{offer.description}</p>
                )}
